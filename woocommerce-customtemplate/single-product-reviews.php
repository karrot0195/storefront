	...
	<div id="comments">
		<h2 class="woocommerce-Reviews-title">
			...
			if ( $count && wc_review_ratings_enabled() ) {
				...
				$reviews_title = sprintf( esc_html( _n( '%1$s review for %2$s', '%1$s reviews for %2$s', $count, 'storefront' ) ), esc_html( $count ), '<span>' . get_the_title() . '</span>' );
		...
		<div id="review_form_wrapper">
			<div id="review_form">
				...
				$comment_form = array(
					...
					'title_reply'         => have_comments() ? __( 'Add a review', 'storefront' ) : sprintf( __( 'Be the first to review &ldquo;%s&rdquo;', 'storefront' ), get_the_title() ),
					/* translators: %s is product title */
				...
				if ( wc_review_ratings_enabled() ) {
					$comment_form['comment_field'] = '<div class="comment-form-rating"><label for="rating">' . esc_html__( 'Your rating', 'storefront' ) . '</label><select name="rating" id="rating" required>
						<option value="">' . esc_html__( 'Rate&hellip;', 'storefront' ) . '</option>
